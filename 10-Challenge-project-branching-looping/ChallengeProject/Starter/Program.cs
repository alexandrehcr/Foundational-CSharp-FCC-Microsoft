    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                // ID index = 0
                // age index = 2
                // physical description index = 4

                // The default ID is an empty string ("")
                if (ourAnimals[i, 0] != "ID #: ")
                {
                    // get only the values present in age and physical description
                    animalID = ourAnimals[i, 0];
                    animalAge = ourAnimals[i, 2].Split(":")[1].Trim();
                    animalPhysicalDescription = ourAnimals[i, 4].Split(":")[1].Trim();
                    ...
                    // if physical description is not set
                    if (animalPhysicalDescription == "tdb" || animalPhysicalDescription == "")
                    {
                        do
                        {
                            validEntry = false;
                            ...
                            {
                                animalPhysicalDescription = readResult.ToLower();
                                int physicalDescriptionLength = animalPhysicalDescription.Length;

                                if (physicalDescriptionLength >= 10)
                                {
                                    // 4 is the physical description index
                                    ourAnimals[i, 4] = "Physical description: " + animalPhysicalDescription;
                                    validEntry = true;
                                }
                                else
                                {
                                    Console.WriteLine("Please provide enough information.");
                                }
                            }
                        } while (!validEntry);
                    }
        ...
        case "4":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                animalID = ourAnimals[i, 0];
                animalNickname = ourAnimals[i, 3].Split(":")[1].Trim();
                animalPersonalityDescription = ourAnimals[i, 5].Split(":")[1].Trim();

                // The default ID is an empty string ("")
                if (animalID != "ID #: ")
                {
                    if (animalNickname == "tbd" || animalNickname == "")
                    {
                        do
                        {
                            validEntry = false;
                    ...
                    if (animalPersonalityDescription == "tbd" || animalPersonalityDescription == "")
                    {
                        do
                        {
                            validEntry = false;
