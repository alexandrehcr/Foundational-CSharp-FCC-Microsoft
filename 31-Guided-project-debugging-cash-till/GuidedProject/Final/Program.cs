...
{
    // this shouldn't be here. The money should be added to the till only once the the amount paid is verified to be greater than the amount needed. Otherwise, the cashier would keep the customer's money when not enough money is provided.
    cashTill[3] += twenties;
    cashTill[2] += tens;
    cashTill[1] += fives;
    cashTill[0] += ones;
