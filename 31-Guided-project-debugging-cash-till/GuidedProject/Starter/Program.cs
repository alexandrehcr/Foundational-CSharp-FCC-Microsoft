...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    // If the code get to here, the payment is enough and there's enough change to return.
    string output = "";
    output += "Cashier Returns:\n";
    output += $"Twenties: {twentiesOfChange}\n";
    output += $"Tens: {tensOfChange}\n";
    output += $"Fives: {fivesOfChange}\n";
    output += $"Ones: {onesOfChange}\n";
    Console.WriteLine(output);

    // Update the till's balance
    cashTill[3] += twenties - twentiesOfChange;
    cashTill[2] += tens - tensOfChange;
    cashTill[1] += fives - fivesOfChange;
    cashTill[0] += ones - onesOfChange;
}
